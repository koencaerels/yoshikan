        ...
        <td v-if="!isView" class="pl-2">
            <span class="p-input-icon-right w-full">
                <Calendar class="p-inputtext-sm w-full"
                          ...
                          dateFormat="dd/mm/yy"/>
                <i v-if="!add$.startDate.$invalid" class="pi pi-check text-green-600"/>
                <i v-if="add$.startDate.$invalid" class="pi pi-times text-red-600"/>
            </span>
        </td>
        <td v-if="!isView" class="pl-2">
            <span class="p-input-icon-right w-full">
                <Calendar class="p-inputtext-sm w-full"
                          ...
                          dateFormat="dd/mm/yy"/>
                <i v-if="!add$.endDate.$invalid" class="pi pi-check text-green-600"/>
                <i v-if="add$.endDate.$invalid" class="pi pi-times text-red-600"/>
            </span>
        </td>
...
import type {AddPeriodCommand} from "@/api/command/addPeriod";
import {addPeriod} from "@/api/command/addPeriod";
...
const command = ref<AddPeriodCommand>({
    name: '',
    code: '',
    startDate: new Date(),
    endDate: new Date()
...
function editForm() {
    ...
    command.value.code = '';
    command.value.name = '';
    command.value.startDate = new Date();
    command.value.endDate = new Date();
}
...
async function save() {
    ...
    if (!add$.value.$invalid) {
        ...
        await addPeriod(command.value);
        toaster.add({
            severity: "success",
            summary: "Periode toegevoegd",
            detail: "",
